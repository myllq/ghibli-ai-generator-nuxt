<template>
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
    ...
    <div class="bg-white p-6 rounded-lg border border-gray-200">
      <h3 class="text-xl font-semibold mb-6 text-[#3d405b]">Generate Ghibli Video</h3>
      ...
      <div class="mb-6">
        ...
        <div
          ...
          @click="!previewUrl && $refs.videoFileUpload.click()"
        ...
        >
          <template v-if="!previewUrl">
            ...
            <input ref="videoFileUpload" type="file" class="hidden" accept="image/*" @change="handleFileChange" />
          </template>
    ...
    <div class="bg-white p-6 rounded-lg border border-gray-200">
      ...
      <div v-if="isGenerating" class="flex flex-col items-center justify-center h-[300px]">
        ...
        <p class="text-gray-500">Generating your Ghibli-style video...</p>
      ...
      <div v-else-if="generatedVideo" class="space-y-4">
        <div class="relative h-[300px] rounded-lg overflow-hidden">
          <img
            :src="generatedVideo"
            alt="Generated Ghibli Video"
            class="absolute inset-0 w-full h-full object-contain"
          />
          <div class="absolute inset-0 flex items-center justify-center">
            <p class="bg-black/70 text-white px-3 py-1 rounded">Video Preview (Static for Demo)</p>
          </div>
        </div>
        <div class="flex justify-center">
          <button class="bg-[#81b29a] hover:bg-[#6a9d87] text-white py-2 px-4 rounded-md w-full flex items-center justify-center" @click="downloadVideo">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="h-4 w-4 mr-2">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
      ...
      <div v-else class="border-2 border-dashed border-gray-300 rounded-lg p-12 text-center h-[300px] flex flex-col items-center justify-center">
        ...
        <p class="text-gray-500 mb-4">Your Ghibli-style video will appear here</p>
...
const prompt = ref("");
const videoFileUpload = ref(null);
...
const generateVideo = () => {
  ...
  setTimeout(() => {
    ...
    generatedVideo.value = `/images/ghibli-image${Math.floor(Math.random() * 3) + 1}-after.webp`;
...
const downloadVideo = () => {
  if (generatedVideo.value && downloadLink.value) {
    // Create a temporary link to download the video
    downloadLink.value.href = generatedVideo.value;
    downloadLink.value.download = `ghibli-video-${Date.now()}.webp`;
